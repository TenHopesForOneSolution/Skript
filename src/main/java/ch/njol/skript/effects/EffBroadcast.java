...
public class EffBroadcast extends Effect {
	...
	public boolean init(Expression<?>[] exprs, int matchedPattern, Kleenean isDelayed, ParseResult parseResult) {
		...
		messages = messageExpr instanceof ExpressionList ? ((ExpressionList<?>) messageExpr).getExpressions()
	...
	public void execute(Event event) {
		...
		for (Expression<?> message : getMessages()) {
			if (message instanceof VariableString variableString) {
				// get both unformatted and components with single evaluation:
				// https://github.com/SkriptLang/Skript/issues/7718
				StringBuilder unformattedString = new StringBuilder();
				List<MessageComponent> messageComponents = variableString.getMessageComponents(event,
						unformattedString);
			...
			} else if (message instanceof ExprColoured coloured && coloured.isUnsafeFormat()) { // Manually marked as
																								// trusted
				...
				for (Object messageObject : message.getArray(event)) {
					String realMessage = messageObject instanceof String string ? string
							: Classes.toString(messageObject);
	...
	private static boolean dispatchEvent(String message, List<CommandSender> receivers) {
		...
		if (Skript.isRunningMinecraft(1, 14)) {
			broadcastEvent = new BroadcastMessageEvent(!FoliaCompat.isTickThread(), message, recipients);
		} else {
	...
	private static @Nullable String getRawString(Event event, Expression<? extends String> string) {
		...
		if (rawString.toLowerCase().contains("&x")) {
			rawString = StringUtils.replaceAll(rawString, HEX_PATTERN,
					matchResult -> "<#" + matchResult.group(1).replace("&", "") + '>');
		}
		return rawString;
	}
