...
public class EvtDamage extends SkriptEvent {
	...
	static {
		Skript.registerEvent("Damage", EvtDamage.class, EntityDamageEvent.class,
				"damag(e|ing) [of %-entitydata%] [by %-entitydata%]")
				.description(
	...
	public boolean check(Event evt) {
		...
		if (e instanceof EntityDamageByEntityEvent
				&& ((EntityDamageByEntityEvent) e).getDamager() instanceof EnderDragon
				&& ((EntityDamageByEntityEvent) e).getEntity() instanceof EnderDragon)
			return false;
	...
	// private final static WeakHashMap<LivingEntity, Integer> lastDamages = new
	// WeakHashMap<LivingEntity, Integer>();
	 ...
	 * Skript.info("is invincible: " + (((LivingEntity) en).getNoDamageTicks() >
	 * ((LivingEntity) en).getMaximumNoDamageTicks() / 2f));
	 * if (((LivingEntity) en).getNoDamageTicks() > 0)
	 * Skript.info("damage difference (positive = more): " + (e.getDamage() -
	 * en.getLastDamageCause().getDamage()));
	 ...
	 * @Override
	 * public void run() {
	 * Skript.info("actual damage: " + (h - Math.max(((LivingEntity)
	 * en).getHealth(), 0)));
	 * }
	 * });
	 * }
